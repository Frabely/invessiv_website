# Plan: Dashboard Shell — Header, Sidebar, Background

## Context

Das Auth-/Routing-Skelett aus `plans/dashboard/clerk-auth-and-shell.md` (Tickets 1–7) hat eine minimale `DashboardShell` produziert, die nur Headline + Sub-Copy auf `/[locale]/dashboard` rendert. Dieser Plan baut die persistente visuelle Chrome (Header, Sidebar, Background) für den geschützten Dashboard-Bereich. Sie wird im gemeinsamen `(dashboard)/layout.tsx` angesiedelt, damit jede künftige Sub-Route automatisch dieselbe Hülle erbt.

**Ergebnis nach diesem Plan:**

- `/[locale]/dashboard` zeigt sticky Header (Brand · Search · Theme · Locale · UserButton) und Sidebar (Desktop: Icon-Rail mit Hover-Expand auf Labels; Mobile: Hamburger + Drawer)
- Inhaltsbereich in der Mitte ist visuell leer, mit reduziertem Hero-Style-Background (Grid + dezente Orange-/Blau-Blobs)
- Brand führt zurück zu `/[locale]/dashboard`
...
- DE und EN parallel im Dashboard-Dictionary gepflegt
- Bestehender Auth-Gate (Layer 2 in `(dashboard)/layout.tsx`) bleibt unangetastet

**Bewusst NICHT in Scope:**

- Echte Navigationsziele oder Sub-Routen (`/[locale]/dashboard/leads` etc.)
...
| Frage                                 | Entscheidung                                                                                                                      |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Shell-Position                        | `src/app/[locale]/(dashboard)/layout.tsx` wickelt `{children}` in `<DashboardShell>` ein                                          |
| Was passiert mit Headline + Sub-Copy? | **Komplett entfernen** (Variante a). Dashboard-Mitte bleibt visuell leer; nur Background sichtbar                                 |
| Header-Inhalte                        | Brand · Search (disabled) · **Theme-Switch** · Locale-Switch · **Clerk `<UserButton>`**                                           |
| Brand-Link Ziel                       | `/[locale]/dashboard` (locale-aware)                                                                                              |
...
| i18n                                  | Erweiterung des bestehenden Dashboard-`page`-Dicts um `shell.*`-Namespace (Header, Sidebar, Search, Main); DE+EN im selben Commit |
...
│   └── [locale]/
│       └── (dashboard)/
│           ├── layout.tsx                          # ÄNDERUNG — wrappt children in <DashboardShell>
│           └── dashboard/
│               └── page.tsx                       # ÄNDERUNG — keine Headline/Sub-Copy mehr, leerer main
│
├── components/
│   ├── dashboard/
│   │   ├── dashboard-shell/                       # ÄNDERUNG
│   │   │   ├── dashboard-shell.tsx                # akzeptiert children, rendert Background-Divs inline + Header/Sidebar/Main
│   │   │   └── dashboard-shell.module.css         # Grid-Layout (Sidebar | Main), Header sticky, Background-Klassen
│   │   ├── dashboard-header/                      # NEU
│   │   │   ├── dashboard-header.tsx               # client wrapper (Hamburger-Toggle), komponiert Brand/Search/Actions
│   │   │   └── dashboard-header.module.css
│   │   ├── dashboard-search/                      # NEU
│   │   │   ├── dashboard-search.tsx               # disabled <input>, Lupen-Icon, „Coming soon"-Hint
│   │   │   └── dashboard-search.module.css
│   │   └── dashboard-sidebar/                     # NEU
│   │       ├── dashboard-sidebar.tsx              # client (hover/focus-within, Drawer)
│   │       ├── dashboard-sidebar.module.css       # Desktop Icon-Rail + Mobile Drawer
│   │       └── dashboard-sidebar-items.ts         # statische Items + Lucide/Inline-SVG-Icons
│
├── hooks/
│   └── dashboard/
│       └── use-dashboard-sidebar-drawer.ts        # NEU — open/close-State, Escape-Listener, Body-Scroll-Lock
│
└── i18n/
    └── dictionaries/
        └── dashboard/
...
- **Mehrere kleine Komponenten statt Monolith**: Header und Sidebar haben unabhängige Lifecycles und Reuse-Pfade (Sidebar bekommt später Items, Header bekommt UserButton-Erweiterungen). Die `DashboardShell` bleibt ein dünner Compositor.
- **Background NICHT als eigene Komponente**: 3 dekorative Divs (2 Blobs + Grid) ohne Logik, ohne Props, einmal verwendet. Eigener Folder + `.tsx` + `.module.css` wäre Over-Engineering. Markup landet inline in `dashboard-shell.tsx`, Styles in `dashboard-shell.module.css`. Falls die CSS-Datei zu groß wird, später nach `dashboard-shell-background.module.css` extrahieren.
- **Hook für Drawer-State**: Hamburger-Button im Header und Drawer in Sidebar müssen synchronisieren. Zwei Optionen — gemeinsamer State im `<DashboardShell>` per `useState` und Prop-Drilling, oder Hook + Context. Wir nehmen **Prop-Drilling**, weil die Komponenten direkt nebeneinander leben und ein Context für einen Boolean Overkill ist. Der Hook kapselt nur Escape-Listener + Body-Scroll-Lock.
- **`dashboard-sidebar-items.ts` statisch**: Items sind heute Platzhalter, später echte Routen. Eigene Datei vermeidet, dass Render-File und Daten-Definition vermischen.

---

## i18n-Plan

### Erweiterung `src/i18n/dictionaries/dashboard/page/de.json`
...
```json
{
  "shell": {
    "header": {
      ...
      "brandHomeAriaLabel": "Zum Dashboard-Start",
      "search": {
        "label": "Leads suchen",
    ...
    "main": {
      "ariaLabel": "Dashboard-Inhalt"
    }
  }
}
```

### Erweiterung `src/i18n/dictionaries/dashboard/page/en.json`
...
```json
{
  "shell": {
    "header": {
      "brandLabel": "Invessiv",
      "brandLogoAlt": "Invessiv logo",
      "brandHomeAriaLabel": "Back to dashboard home",
      "search": {
        "label": "Search leads",
...
- **`headline` und `subCopy` werden komplett entfernt**, weil sie ab diesem Ticket nicht mehr gerendert werden. Type `DashboardPageContent` leitet sich von `de.json` ab — Tests/Components, die noch auf die Felder zugreifen, müssen mit angepasst werden.
...
> Alle Tickets folgen `plans/dashboard/clerk-auth-and-shell.md` Abschnitt 2 (ein Ticket pro Schritt, Plan-Modus zuerst, Commits nur auf Ansage).

### Ticket 1 — Dashboard-Copy strukturieren

- `src/i18n/dictionaries/dashboard/page/{de,en}.json`: den `shell`-Namespace ergänzen und `headline` sowie `subCopy` entfernen.
- `src/i18n/dictionaries/dashboard/index.ts`: keine API-Änderung nötig; `DashboardPageContent` bleibt aus `de.json` abgeleitet.
- Verbleibende Zugriffe auf `content.headline` und `content.subCopy` identifizieren und für die Folge-Tickets markieren, mindestens in `dashboard-shell.tsx`, `dashboard/page.tsx` und den zugehörigen Tests.
...
### Ticket 2 — `DashboardSearch`

- Neue Komponente `src/components/dashboard/dashboard-search/dashboard-search.tsx` (kann Server Component bleiben — keine Interaktion).
...
### Ticket 3 — `useDashboardSidebarDrawer` Hook

- `src/hooks/dashboard/use-dashboard-sidebar-drawer.ts`:
...
- Unit-Test `use-dashboard-sidebar-drawer.test.ts` (Vitest + Testing Library): open/close/toggle, Escape, Body-Lock-Effect.
- **Acceptance:** Tests grün, Hook hat keine Memory-Leaks (Cleanup im `useEffect`).

### Ticket 4 — `DashboardSidebar`

- `src/components/dashboard/dashboard-sidebar/dashboard-sidebar-items.ts`:
  - Statisches Array `[{ id, labelKey, iconSvg }]` für `leads` und `overview`.
  - Icons als inline SVG-Komponenten (analog `LocaleSwitch`).
- `src/components/dashboard/dashboard-sidebar/dashboard-sidebar.tsx` (`"use client"`):
  - Props: `{ content, isOpen, onCloseAction }` (Drawer-State kommt vom Header via `DashboardShell`).
...
### Ticket 5 — `DashboardHeader`

- `src/components/dashboard/dashboard-header/dashboard-header.tsx` (`"use client"`, da `useLanguage`/`useTheme` Hooks verbraucht):
  ...
  - Brand: `<a>` mit `next/image` (`/brand/icon.png`, 26×26, `priority`) + Span. `href={\`/${locale}/dashboard\`}`.
...
### Ticket 6 — `DashboardShell` als Wrapper (inkl. Background)

- Refactor `src/components/dashboard/dashboard-shell/dashboard-shell.tsx`:
  - Wird `"use client"` (wegen Drawer-State).
  - Props: `{ content: DashboardPageContent, locale: Locale, children: ReactNode }`.
  - `useDashboardSidebarDrawer()` für Drawer-State.
  - Komposition:
    ```tsx
...
- `src/app/[locale]/(dashboard)/layout.tsx`:
  - Nach `await requireDashboardAccess(activeLocale)` Dictionary laden: `const content = getDashboardPageContent(activeLocale);`
  - `<DashboardShell content={content} locale={activeLocale}>{children}</DashboardShell>` zurückgeben.
  - Bisherige `<main id="main-content">{children}</main>` wird entfernt — der `<main>` lebt jetzt **innerhalb** der Shell.
- `src/app/[locale]/(dashboard)/dashboard/page.tsx`:
  - `<DashboardShell content={...} />` Aufruf entfernen.
...
- **Acceptance:** `/de/dashboard` und `/en/dashboard` zeigen die Shell mit leerer Mitte; `noindex` bleibt; `dynamic = "force-dynamic"` bleibt; Auth-Gate bleibt aktiv.

### Ticket 8 — Tests + Pre-merge Gates

- `src/hooks/dashboard/use-dashboard-sidebar-drawer.test.ts`: open/close/toggle, Escape, Body-Lock.
- Bestehender Test `dashboard-shell.test.*` (falls vorhanden) anpassen oder löschen — neue, kleinere Tests:
  - `dashboard-search.test.tsx`: Disabled-State, Hint sichtbar, kein Submit möglich.
  - `dashboard-sidebar.test.tsx`: Mobile-Drawer-Toggle via Prop, disabled Items skippen Tab-Order.
- E2E (optional, nur wenn schnell machbar): `e2e/dashboard-shell.e2e.ts` — eingeloggter Owner sieht Header + Sidebar, Hamburger toggelt.
...
| --- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 1   | Owner einloggen → `/de/dashboard`                  | Header (Brand + Search disabled + Theme + Locale + UserButton) sichtbar; Sidebar als Icon-Rail links      |
...
| 4   | Locale-Switch DE → EN                              | URL bleibt `/dashboard`, alle Header-/Sidebar-Texte werden Englisch, kein Layout-Sprung                   |
...
| 10  | Brand-Klick                                        | Navigiert zu `/[currentLocale]/dashboard`                                                                 |
| 11  | View-Source `/de/dashboard`                        | `<meta name="robots" content="noindex,nofollow">` weiterhin gesetzt                                       |
...
| 13  | Nicht eingeloggter User → `/de/dashboard`          | Redirect zu `/de/sign-in?redirect_url=...` (Auth-Gate aus Layer 1 unverändert)                            |
| 14  | Eingeloggter User ohne Allowlist → `/de/dashboard` | HTTP 404 (Auth-Gate aus Layer 2 unverändert)                                                              |
...
- `npm run build` → grün, kein Static-Generation-Versuch für Dashboard-Page
...
| Locale-Switch im Dashboard wechselt Pfad nicht korrekt (`/de/dashboard` ⇄ `/en/dashboard`) |  mittel | Logik aus `site-header.tsx` 1:1 übernehmen (Pfad-Segment-Replace). Manuell testen in Szenario 4                                               |
...
| Feature                                 | Trigger                                                     | Wo                                                                              |
| --------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Echte Sidebar-Routen                    | Sobald erste Sub-Route existiert (z. B. `/dashboard/leads`) | `dashboard-sidebar-items.ts` mit Hrefs befüllen, `aria-disabled` entfernen      |
| Funktionierende Lead-Suche              | Sobald Lead-DB-Schema und API stehen                        | `dashboard-search.tsx` zu `"use client"` mit Debounce + Server-Action           |
...
| Breadcrumbs in Header                   | Sobald >1 Sub-Route navigierbar ist                         | Eigene Komponente `dashboard-breadcrumbs/`, in Header zwischen Brand und Search |
...
- `src/app/[locale]/(dashboard)/CLAUDE.md` — Dashboard-Architektur, Schutz-Layer, geplante Erweiterungen
- `src/app/[locale]/(dashboard)/AGENTS.md` — Dashboard-Pflichtregeln (Auth, i18n, Komponenten)
...
- `plans/dashboard/clerk-auth-and-shell.md` — Vorgänger-Plan, Auth-/Routing-Skelett
