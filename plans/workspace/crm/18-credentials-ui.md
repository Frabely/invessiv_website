...
> **Migration:** `0028_create_customer_credentials.sql` (Planwert)
...
| Protokoll               | Jede Aufdeckung schreibt eine `customer_activities`-Zeile mit Bezeichnung des Zugangs und Zeitpunkt — **nie** mit dem Wert |
...
POST   /api/workspace/crm/credentials/[credentialId]/reveal
         ...
         → schreibt customer_activities (credential_revealed)
...
```txt
packages/db/migrations/0028_create_customer_credentials.sql
...
- **Files:** `0028_create_customer_credentials.sql`, `record-configuration/crm/customer-credentials.ts`,
