...
> **Migration:** `0031_create_conversations.sql` (Planwert)
...
| Lesestand                          | Zwei Zeitstempel an der Unterhaltung: `internal_read_at` und `customer_read_at`. Ungelesen ist alles nach dem jeweiligen Zeitstempel                              |
...
| Bearbeiten und Löschen             | Eigene Nachricht innerhalb von 15 Minuten bearbeitbar, danach nicht mehr. Löschen ist ein Soft-Delete (`deleted_at`), der Verlauf zeigt „Nachricht zurückgezogen" |
...
messages
  ...
  INDEX (customer_id) WHERE deleted_at IS NULL
```
...
```txt
packages/db/migrations/0031_create_conversations.sql
...
- **Files:** `0031_create_conversations.sql`, zwei `pgTable`-Dateien,
...
- **Inhalt:**
  ...
  - Ungelesen-Zählung je nach abfragender Seite: Nachrichten der **anderen** Seite nach dem eigenen
    Lesestand
  - Gelöschte Nachrichten kommen mit, aber ohne Inhalt (`body` leer, `deletedAt` gesetzt)
- **Akzeptanz:**
...
### CRM-24-T3 — Senden, Bearbeiten, Löschen
...
- **Inhalt:**
  ...
  - Bearbeiten: nur eigene Nachricht, nur binnen 15 Minuten, nur Typ `text`, setzt `edited_at`
  - Löschen: nur eigene, Soft-Delete, Inhalt wird überschrieben statt nur markiert
...
- **Akzeptanz:**
  ...
  - Test: Systemnachricht lässt sich weder bearbeiten noch löschen
...
2. Nachrichten lassen sich über die Handler senden, bearbeiten und löschen — jeweils mit korrekten
   Grenzen.
