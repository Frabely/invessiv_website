...
> **Migration:** `0027_create_files.sql` (Planwert)
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Zuordnung                   | Polymorph über `owner_type` (`customer`, `project`, `submission`) plus `owner_id`, dazu **immer** `customer_id` als denormalisierte Spalte                      |
| Warum `customer_id` doppelt | Damit jede Rechte- und Portalabfrage ohne Join auf den Besitzer filtern kann. Sicherheitsprüfungen sollen nicht von einem korrekten Join abhängen               |
...
files
  ...
  owner_type text NOT NULL                CHECK in ('customer','project','submission')
...
1) POST /api/workspace/crm/files/ticket
     ...
     → zod: owner, Dateiname, MIME, Größe
...
3) POST /api/workspace/crm/files/complete
     ...
     → files insert
     → customer_activities: file_uploaded
     → FileDto
...
```txt
packages/db/migrations/0027_create_files.sql
packages/db/src/record-configuration/crm/files.ts
packages/common/src/constants/crm/{file-categories,file-owner-types}.ts
...
apps/workspace/src/server/workspace/crm/
  ...
  services/file-ownership-service.ts        prüft, dass owner zum customer gehört
...
- **Files:** `0027_create_files.sql`, `record-configuration/crm/files.ts`,
  `constants/crm/{file-categories,file-owner-types}.ts` + Tests, `contracts/crm/file.dto.ts`
...
- **Akzeptanz:** Migration idempotent; doppelter `storage_key` wird abgelehnt
...
- **Skills:** `best-practices`
- **Inhalt:**
  - `listFiles({ customerId, ownerType?, ownerId?, category? })`
