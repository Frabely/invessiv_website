...
| Spalten         | Firma, Status, Ort, Primärkontakt, Letzte Änderung                                                                                             |
...
apps/workspace/src/components/workspace/crm/
  ...
  table/customers-pagination/**
  table/customers-empty-state/**
...
- **Inhalt:**
  - `listCustomers({ page, sort })` mit Drizzle: Join auf Primärkontakt, `count` für Pagination,
    Sortierung nach Firma / Status / Änderungsdatum
...
- **Inhalt:**
  - Server-Table + Client-Row nach dem Leads-Muster, co-located `*.module.css`, Zustände über
    `data-*`-Attribute, Farben über bestehende Theme-Tokens
