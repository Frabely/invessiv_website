...
> **Migration:** `0023_create_customer_tags.sql` (Planwert)
...
| Statuswechsel-Protokoll | Jeder Wechsel schreibt eine `customer_activities`-Zeile vom Typ `status_change` mit altem und neuem Wert |
...
```txt
packages/db/migrations/0023_create_customer_tags.sql
...
- **Files:** `0023_create_customer_tags.sql`, zwei `pgTable`-Dateien, Barrel-Erweiterung
