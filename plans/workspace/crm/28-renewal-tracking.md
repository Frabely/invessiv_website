...
- **Files:** `api/cron/renewals/route.ts`, `services/renewal-reminder-service.ts`,
  `vercel.json`, `.env.example` (`CRON_SECRET`) + Tests
...
- **Inhalt:**
  ...
  - Cron-Eintrag in `vercel.json`, täglich morgens
- **Akzeptanz:**
...
2. **Bricht nichts:** eine neue Tabelle. Der Cron-Endpunkt ist durch das Token geschützt und tut ohne
   Einträge nichts. `vercel.json` entsteht neu — bisher existiert keine; der Eintrag betrifft
