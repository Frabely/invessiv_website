# Plan: Clerk Auth + geschützter `/[locale]/workspace` Bereich

## Context

Die öffentliche Marketing-Seite invessiv.com soll um einen privaten Workspace-Bereich unter `/[locale]/workspace` ergänzt werden. **Dieses Ticket implementiert NUR das Auth-/Routing-Skelett** — kein echter Workspace-Inhalt. Ziel ist eine saubere, erweiterbare Struktur, damit später keine großen Refactors nötig sind.

**Was am Ende dieses Tickets funktioniert:**

- `/[locale]/workspace` ist erreichbar, aber serverseitig geschützt
...
- Allowlistete User sehen einen leeren Workspace-Placeholder
...
- Workspace-spezifische `AGENTS.md` + `CLAUDE.md` auf Routen-Ebene

**Was bewusst NICHT implementiert wird:**

- Kein echter Workspace-Inhalt, keine Sidebar, keine Widgets
...
- Keine Tests für Workspace-Inhalte (es gibt keine Inhalte)
...
| `frontend-design`                 |                    required | Tickets 6-7       | Auth-Frame und Workspace-Shell sind sichtbare UI; Umsetzung muss konsistent, responsiv, theme-fähig und nicht generisch sein        |
| `copywriting`                     | required bei Textänderungen | Tickets 6-7       | Sign-in-/Sign-up-Frame, Workspace-Placeholder, Meta-Texte und Microcopy müssen in DE/EN parallel sauber formuliert werden           |
...
| `seo`                             |                    sinnvoll | Tickets 7, 9      | Workspace muss bewusst `noindex`, `nofollow`, `nocache` und ohne Sitemap-Eintrag bleiben; Auth-Seiten brauchen konsistente Metadata |
...
| Root `CLAUDE.md` / `AGENTS.md` | Verweisen inzwischen auf `(auth)` und `workspace` Scope-Dateien                                                                  |
| `src/app/CLAUDE.md`            | App-Router-Guidance auf `(auth)` für öffentliche Auth-Routen und `workspace` für geschützte Workspace-Flows aktualisiert         |
...
| Nicht eingeloggt → Workspace | Redirect zu `/[locale]/sign-in?redirect_url=...`                                                                                                         |
| Eingeloggt, nicht erlaubt    | `notFound()` → HTTP 404                                                                                                                                  |
| Nav-Visibility               | **Komplett versteckt** — kein Link in Header/Footer, nur Direkt-URL                                                                                      |
| Workspace i18n               | Volles i18n via `src/i18n/dictionaries/workspace/...`                                                                                                    |
...
| AGENTS.md/CLAUDE.md Scope    | `src/app/[locale]/(auth)/AGENTS.md`, `src/app/[locale]/(auth)/CLAUDE.md`, `src/app/[locale]/workspace/AGENTS.md`, `src/app/[locale]/workspace/CLAUDE.md` |
| Auth-Page Robots             | Auth-Seiten sind keine SEO-Landingpages und werden standardmäßig `noindex, follow: false` gesetzt                                                        |
| Sign-up Default              | `/[locale]/sign-up` bleibt zunächst erreichbar; Nutzer ohne Allowlist-Match sehen nach Login weiterhin 404 im Workspace                                  |
...
│   │   │
│   │   ├── workspace/                           # NEU — geschützte App-/Workspace-Routen
│   │   │   ├── AGENTS.md                    # NEU — workspace-scope Agent-Regeln
│   │   │   ├── CLAUDE.md                    # NEU — workspace-scope Architektur-Knowledge
...
│   ├── workspace/                           # NEU — vorbereitend (heute nur Shell)
│   │   └── workspace-shell/
│   │       ├── workspace-shell.tsx          # leerer Placeholder mit i18n-Headline
...
│       ├── permissions.ts                   # requireWorkspaceAccess() — auth() + allowlist + redirect/notFound
│       └── routes.ts                        # Konstanten: SIGN_IN_PATH, WORKSPACE_PATH, AFTER_SIGN_IN_PATH (locale-aware)
│
├── i18n/
│   └── dictionaries/
│       ├── auth/                            # NEU
...
- **`workspace` URL-Segment**: Genau wie in der vorhandenen `CLAUDE.md` vorgesehen. `layout.tsx` im Segment ist der zentrale Auth-Gate — alle künftigen Workspace-Subroutes erben den Schutz automatisch.
- **`src/lib/auth/`**: Server-only Helpers, klar abgegrenzt von Client-Auth-Hooks (die kommen später bei Bedarf in `src/hooks/auth/`).
- **`src/components/workspace/`**: Eigene Komponenten-Domain analog zu `marketing/`, `legal/`, `shared/`. Skaliert sauber, wenn später Sidebar/Widgets dazukommen.
...
- **AGENTS.md/CLAUDE.md auf `workspace/` Ebene**: Greift für den gesamten geschützten Workspace-Subtree (`workspace/`, später `settings/`, etc.) und alle Allowlist-/Permission-Grenzen.
...
| `NEXT_PUBLIC_CLERK_SIGN_IN_FALLBACK_REDIRECT_URL` | `.env.local`, Vercel                            | `/workspace`                                         |
| `NEXT_PUBLIC_CLERK_SIGN_UP_FALLBACK_REDIRECT_URL` | `.env.local`, Vercel                            | `/workspace`                                         |
...
Wenn eine `.env.example` im Rahmen dieses Plans angelegt wird, darf sie nicht nur die neuen Clerk-/Workspace-Variablen enthalten. Sie muss alle aktuell in den lokalen ENV-Dateien bekannten Key-Namen dokumentieren:
...
# Workspace Allowlist (Komma-getrennt, lowercase, ohne Whitespace)
...
const isProtectedRoute = createRouteMatcher(["/:locale/workspace(.*)"]);
...
export default clerkMiddleware(async (auth, req) => {
  ...
  // 2) Auth-Schutz nur für Workspace
...
**Wichtig**: `auth.protect()` deckt nur "nicht eingeloggt" ab. Die Allowlist-Prüfung passiert **zusätzlich** im Server-Layout (`workspace/layout.tsx`) — siehe 8.3. Doppelter Check ist Absicht: Defense-in-Depth.
...
export async function requireWorkspaceAccess(locale: SupportedLocale) {
...
Aufruf in `src/app/[locale]/workspace/layout.tsx` — gilt damit für alle Workspace-Subroutes.
...
- **Rollen** (z. B. `admin`, `viewer`): Migration auf Clerk `publicMetadata.role`. `permissions.ts` bekommt `requireRole(role)`-Variante. Schema im Workspace-CLAUDE.md vorgemerkt.
- **DB-basierte ACL**: Wenn >5 User oder dynamische Permissions: Drizzle-Tabelle `workspace_access` in `src/server/db/record-configuration/`. Erst dann nötig.
...
| `/[locale]/workspace`                     | geschützt  | Auth + Allowlist                                             |
| `/api/public/contact`                     | öffentlich | unverändert                                                  |

**Verhalten am `/[locale]/workspace`-Endpoint:**

| User-Zustand                       | Reaktion                                                                                          |
| ---------------------------------- | ------------------------------------------------------------------------------------------------- |
| Nicht eingeloggt                   | `auth.protect()` in `proxy.ts` → Redirect zu `/[locale]/sign-in?redirect_url=/[locale]/workspace` |
| Eingeloggt, **nicht** in Allowlist | `requireWorkspaceAccess()` in Layout → `notFound()` → HTTP 404                                    |
| Eingeloggt, in Allowlist           | Workspace-Shell wird gerendert                                                                    |
...
- `/de/workspace` ohne Login redirectet zu `/de/sign-in?redirect_url=%2Fde%2Fworkspace`.
- `/en/workspace` ohne Login redirectet zu `/en/sign-in?redirect_url=%2Fen%2Fworkspace`.
- Nach erfolgreichem Login führt `redirect_url` zurück zur ursprünglichen Locale-Workspace-URL.
- Fallback-Redirects aus Clerk-ENV dürfen nie auf nicht-lokalisierte `/workspace`-URLs führen; App-Code ergänzt bei Bedarf den Locale-Prefix.
...
**Entscheidung: Workspace ist komplett versteckt.**
...
**Konsequenz:** Erreichbarkeit nur durch direktes Tippen von `/de/workspace` oder Bookmark. Logout passiert auf der Workspace-Seite selbst (in einem späteren Ticket via `<UserButton>` oder eigener Logout-Button — heute nicht im Scope).
...
## 11. Workspace-Shell-Plan

**`src/app/[locale]/workspace/page.tsx`** — leer im Sinne von "kein Inhalt", aber strukturell vollständig:
...
- Rendert nur `<WorkspaceShell />`-Komponente mit Headline aus i18n-Dictionary

**`src/app/[locale]/workspace/layout.tsx`**:

- Server Component
- Ruft `await requireWorkspaceAccess(locale)` auf — alle Auth-Logik zentral hier
- Rendert `{children}` (heute = Workspace-Shell, später = Shell mit Sidebar)

**`src/components/workspace/workspace-shell/`** — leerer Placeholder:

- Headline aus Dict (z. B. "Invessiv Workspace"), Sub-Copy "Bereich wird vorbereitet"
...
- `src/app/[locale]/workspace/AGENTS.md` — gilt für den gesamten Workspace-Subtree.
- `src/app/[locale]/workspace/CLAUDE.md` — Architektur-Knowledge für den Workspace.
...
Status: erledigt durch diesen Plan, Auth-/Workspace-Scope-Dateien und Root-Verweise:

- `src/app/[locale]/(auth)/AGENTS.md`
- `src/app/[locale]/(auth)/CLAUDE.md`
- `src/app/[locale]/workspace/AGENTS.md`
- `src/app/[locale]/workspace/CLAUDE.md`
- Root `AGENTS.md` und Root `CLAUDE.md` mit Scope-Index
- `src/app/CLAUDE.md` mit korrigierter Konvention für `(auth)` und `workspace`

### Ticket 2 — Clerk Dependency + ENV vorbereiten [erledigt]

- `npm install @clerk/nextjs @clerk/localizations`
- Falls eine `.env.example` angelegt wird: vollständig mit allen Key-Namen aus Abschnitt 7, plus neuen Clerk-/Workspace-Keys, nur mit Platzhalterwerten
...
- `src/lib/auth/routes.ts` (`signInPathFor(locale)`, `WORKSPACE_PATH`, etc.)
- **Acceptance**: `npm run test` grün, Unit-Tests decken Allowlist-Edge-Cases ab (leerer String, Whitespace, Mixed-Case, mehrere Einträge), Route-Helper (`signInPathFor`, `signUpPathFor`, `workspacePathFor`) und `requireWorkspaceAccess()`-Pfade (unauthenticated redirect, missing email, denied email, allowed email).
...
- **Acceptance**: Legacy-Redirects funktionieren weiter (e2e-Test bestehend bleibt grün), `/de/workspace` ohne Login → Redirect zu `/de/sign-in?redirect_url=...`, `/en/workspace` ohne Login → Redirect zu `/en/sign-in?redirect_url=...`, Post-login Redirect bleibt in derselben Locale, kein Fallback auf nicht-lokalisiertes `/workspace`.
...
### Ticket 7 — Workspace-Shell

- `src/app/[locale]/workspace/layout.tsx` mit `requireWorkspaceAccess()`
- `src/app/[locale]/workspace/page.tsx` (`metadata` mit noindex, `dynamic = "force-dynamic"`, ruft `<WorkspaceShell />`)
- `src/components/workspace/workspace-shell/workspace-shell.tsx` (+ `.module.css`) mit i18n-Headline
- `src/i18n/dictionaries/workspace/meta/{de,en}.json` + `page/{de,en}.json` + `index.ts`
...
- README-/CLAUDE.md-Eintrag im Root mit kurzem Hinweis auf Auth- und Workspace-Scope-Dateien, falls durch Ticket 1 noch nicht ausreichend vorhanden
...
| --- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 1   | Nicht eingeloggt → `/de/workspace` aufrufen                    | Redirect zu `/de/sign-in?redirect_url=%2Fde%2Fworkspace`                            |
| 2   | Mit Test-User (nicht in Allowlist) einloggen → `/de/workspace` | Nach Login: HTTP 404 (`notFound()`)                                                 |
| 3   | Mit `moritz-hecht@gmx.net` einloggen → `/de/workspace`         | Workspace-Placeholder-Headline auf Deutsch sichtbar                                 |
| 4   | Mit Owner einloggen → `/en/workspace`                          | Placeholder auf Englisch                                                            |
| 5   | `/de` öffnen, Header anschauen                                 | KEIN Workspace-Link sichtbar (egal ob eingeloggt)                                   |
...
| 9   | DevTools Network-Tab auf `/de/workspace`                       | Response-Header `Cache-Control: no-store` (oder gleichwertig durch Next.js dynamic) |
| 10  | View-Source auf `/de/workspace`                                | Meta `<meta name="robots" content="noindex,nofollow">`                              |
...
- `npm run build` → grün, keine Warnings zu Static Generation der Workspace-Page

### Vercel-ENV-Check (nach Deploy)

- Preview-Deployment einer feature-branch öffnen
- `/de/workspace` ohne Login → Redirect zur Sign-in
...
| Static Generation für Workspace                                             | niedrig | `dynamic = "force-dynamic"` setzt das aus, Build sollte bei generateStaticParams nicht versuchen die geschützte Seite zu rendern (Clerk wirft auf Build-Time keinen User). Verifizieren in Ticket 7.                                                                                                                                                                      |
...
- **Falsches Segment-/Route-Group-Naming** (`workspace` vs. `(app)` vs. `(private)`) → wir bleiben bei `workspace` als URL-Segment (Owner-Entscheidung). `src/app/CLAUDE.md` ist im Rahmen von Ticket 1 auf `(auth)` und `workspace` korrigiert.
...
1. **Sign-up bleibt öffentlich erreichbar.** `/[locale]/sign-up` wird umgesetzt. Nutzer ohne Allowlist-Match werden nach Login beim Workspace-Zugriff weiterhin per 404 abgefangen. Falls strikteres Onboarding gewünscht ist, wird Sign-up im Clerk Dashboard deaktiviert; möglichst ohne Code-Änderung.
