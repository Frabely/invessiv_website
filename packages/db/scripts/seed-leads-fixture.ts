...
function deriveLeadDisplayName(input: {
  company_name?: string | null;
  first_name?: string | null;
  last_name?: string | null;
}): string | undefined {
  const companyName = trimValue(input.company_name);
  if (companyName) {
    return companyName;
  }

  const fullName = [trimValue(input.first_name), trimValue(input.last_name)]
    .filter(Boolean)
    .join(" ")
    .trim();

  return fullName || trimValue(input.last_name);
}
...
function normalizeLeadProfileUrl(url: string): string {
