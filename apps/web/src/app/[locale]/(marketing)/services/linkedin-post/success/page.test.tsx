...
import LinkedInPostSuccessRoute, { generateMetadata } from "./page";
...
describe("LinkedInPostSuccessRoute", () => {
  beforeEach(() => {
  ...
  it("renders the minimal header, success content, and footer for supported locales", async () => {
    const { container } = render(
      await LinkedInPostSuccessRoute({
    ...
    expect(back.getAttribute("href")).toBe("/de/services/linkedin-post");
  ...
  it("returns notFound for unsupported locales", async () => {
    await expect(
      LinkedInPostSuccessRoute({ params: Promise.resolve({ locale: "fr" }) }),
