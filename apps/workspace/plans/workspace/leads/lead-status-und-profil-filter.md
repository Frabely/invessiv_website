...
| Messaging-Auswertung  | Nur `follow_up` und `reminder` zählen zusätzlich zur Basis „Kontaktiert“; die übrigen Outreach-Status bleiben außerhalb der Auswertung              |
...
- **Server:** Create/Update/Bulk-Edit/Import/Messaging-Auswertung
...
### 4. Import und Messaging-Auswertung

- Import-Validierung und Import-Command auf die neuen Stati prüfen
- Messaging-Auswertung: `follow_up` und `reminder` werden der kumulativen Basis „Kontaktiert“ zugerechnet, erhöhen aber
  nicht die Stufe „Geantwortet“. `connection_requested`, `connected` und `not_reached` bleiben außerhalb.
- Mapper-Tests sichern sowohl die einbezogenen als auch die ausgeschlossenen Outreach-Status ab.
