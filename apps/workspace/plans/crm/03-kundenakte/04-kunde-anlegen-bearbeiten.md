...
const createCustomerSchema = z.object({
  ...
  status: z.enum(CUSTOMER_STATUS_VALUES).default(CustomerStatus.Onboarding),
...
GET  /api/workspace/crm/customers/similar?name=…
  ...
  → findSimilarCustomers: bis zu 5 Treffer über unscharfen Namensvergleich, deleted_at IS NULL
```
...
- **Inhalt:**
  ...
  - Transaktion inklusive Activity-Eintrag über `activityService.createActivity` (Task 01a) —
    **kein** direkter Insert in die Aktivitätentabelle
  - `findSimilarCustomers(name)`: unscharfer Vergleich über `lower(btrim(...))` auf Anzeigename und
    Firmenname, maximal fünf Treffer, `deleted_at IS NULL`
- **Akzeptanz:**
