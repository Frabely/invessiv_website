...
> **Verbindliche Revision 2026:** Gehört zu Merge-Einheit 12. Der Portalzugang unterstützt ab
> Version 1 mehrere Firmen je Clerk-Konto; jeder E-Mail-basierte automatische Abgleich ist verboten.

## Verbindliche Revision

- Invitation bindet eine `customer_contact_assignment_id`; gespeichert wird nur ein Tokenhash.
- Token sieben Tage gültig, einmal nutzbar, widerrufbar und niemals in Logs.
- `portal_memberships` verbindet Clerk-User, Person und Kunde. Eine Person darf beliebig vielen
  Kunden angehören; ein Kunde darf mehrere Kontakte besitzen.
- Portal-Mitgliedschaften haben in Version 1 keine differenzierten Rollen: alle erhalten denselben
  fachlichen Umfang. Interne Notizen, Audit, Credentials und Finanzwerte bleiben ausgeschlossen.
- Firmenwechsler ist Bestandteil dieses Tasks. Aktiver Kunde wird in serverseitig signierter Sitzung
  geführt und bei jedem Handler gegen die Mitgliedschaft geprüft.
- Widerruf wirkt sofort; historische Nachrichten bleiben erhalten.
- Vor der ersten Einladung ist eine interne Portalvorschau zu bestätigen.
- Branch `feat/crm-portal-identitaet`.
...
- **Inhalt:**
  - `resolvePortalActor`: **alle** Zeilen zur kleingeschriebenen E-Mail laden. Genau eine nicht
    widerrufene Zeile ergibt den Actor; mehrere ergeben einen eigenen Fehlerzustand (`PortalAccessAmbiguous`), keine
    stille Auswahl. Kennung beim ersten Mal nachtragen, Status auf
...
- **Skills:** `best-practices`
- **Inhalt:**
  - Einladen prüft: Adresse nicht intern, bei diesem Kunden nicht bereits vergeben, Kunde existiert. Eine Adresse, die
    bereits bei einem anderen Kunden Portalzugang hat, wird mit einem eigenen Hinweis abgelehnt — solange es keinen
    Kundenwechsler gibt
...
6. Dieselbe Adresse lässt sich vorerst nicht zwei Kunden zuordnen — die Ablehnung erklärt, dass das eine Produktgrenze
   ist und keine technische.
