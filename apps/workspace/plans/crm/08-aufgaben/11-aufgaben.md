...
- Genau ein Kontext: intern (beide FKs null), Kunde oder Projekt. Projektaufgabe speichert Kunde und
  erhält zusammengesetzte FK-Konsistenz zu `(projects.id, projects.customer_id)`.
- Aufgaben sind flach: kein Parent, keine Unteraufgaben, keine Checklistenpunkte und kein `sort_order`.
...
| Handlungspflicht     | `action_side`: `internal` oder `customer`; Bearbeiter bleibt immer intern                                                                                                 |
...
tasks
  ...
  action_side text NOT NULL DEFAULT 'internal'            CHECK in ACTION_SIDE_VALUES
  assignee_member_id uuid NOT NULL REFERENCES workspace_members(id)
  visible_to_customer boolean NOT NULL DEFAULT false
  due_on date NULL
  done_at timestamptz NULL
  done_by_side text NULL                              CHECK in ACTION_SIDE_VALUES
...
1. Aufgaben lassen sich am Kunden und am Projekt anlegen, bearbeiten und abhaken; kein Delete in v1.
