...
| Gelöschte      | Aufgaben gelöschter Kunden und Projekte erscheinen nicht (Filter über `deleted_at` des Parents)                            |
| Kundenaufgaben | Aufgaben mit `responsible_side = customer` erscheinen mit, aber optisch klar als Bringschuld des Kunden markiert           |
...
- **Inhalt:**
  - `customerId` wird optional; neue Filter `responsibleSide`, `projectId`, `dueWithinDays`,
    `overdueOnly`, `open`
  - Join auf Kunde und Projekt für die Anzeigespalten, `deleted_at IS NULL` auf beiden
