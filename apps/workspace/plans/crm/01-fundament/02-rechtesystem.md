...
`resolveWorkspaceActor`. Für allowlisted Nutzer ändert sich dadurch **nichts** — solange keine
`workspace_members`-Zeile ohne Allowlist-Eintrag existiert, ist das Verhalten bitgleich zu heute.
...
3. **Offen:** Rollenvergabe-UI. Nicht nötig — Rollen werden vorerst per SQL gesetzt, der Bootstrap
   deckt den Alltagsfall ab. Ein UI kommt erst, wenn ein zweiter interner Nutzer existiert. Weil die
   DB-Zeile allein den Zugang öffnet, braucht es dafür dann keinen Deploy mehr.
