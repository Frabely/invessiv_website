...
export async function createQuickContactNotificationMessage(
  ...
  return {
    html: [
      ...
      `<tr><td style="padding:6px 12px 6px 0;font-weight:600;vertical-align:top;">${escapeHtml(copy.nameLabel)}:</td><td style="padding:6px 0;">${escapeHtml(payload.fullName)}</td></tr>`,
    ...
    subject: `${environmentPrefix}[${copy.subjectPrefix}] ${sanitizeLine(payload.fullName)}`,
    text: [
      copy.heading,
      "",
      `${copy.nameLabel}: ${payload.fullName}`,
