...
describe("lead list dialog query helpers", () => {
  it("builds a create dialog URL with mode=create", () => {
    const href = buildLeadCreateHref("/de/workspace/leads", {
      status: "qualified",
    });
  ...
  it("builds an edit dialog URL with mode=edit and the lead id", () => {
    const href = buildLeadEditHref("/de/workspace/leads", "lead-123", {
      selected: "lead-999",
      status: "qualified",
    });
    const url = new URL(href, "https://invessiv.com");
  ...
  it("builds a table-row edit dialog URL from the current query string", () => {
    const href = buildLeadTableRowEditHref("/de/workspace/leads", "lead-123", {
      selected: "lead-999",
      status: "qualified",
    });
    const url = new URL(href, "https://invessiv.com");
  ...
  it("removes dialog state when closing but keeps the surrounding list state", () => {
    const href = buildLeadDialogCloseHref("/de/workspace/leads", {
      edit: "lead-123",
      mode: "edit",
      selected: "lead-999",
      status: "qualified",
