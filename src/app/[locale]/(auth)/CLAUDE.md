...
Der Frame gehört als wiederverwendbare Komponente nach `src/components/auth/auth-frame/`. Page-Dateien bleiben dünn und orchestrieren nur.
...
Clerk-interne UI-Strings werden über `@clerk/localizations` bereitgestellt. Eigene deutsche und englische Texte müssen immer parallel gepflegt werden.

Keine locale-basierten Inline-Branches in Pages, Komponenten, Provider-Konfigurationen oder Lib-Code. Auch wenn aktuell nur DE/EN vorgesehen sind, wird immer so modelliert, als könnten weitere Sprachen folgen. Muster wie `locale === "de" ? deDE : enUS` sind nicht zulässig; stattdessen zentrale Lookup-Strukturen wie `Record<SupportedLocale, ClerkLocalization>` nutzen.
...
Auth-Seiten sind keine Landingpages. Metadata soll eindeutig sein, aber nicht auf organische Suche optimiert werden. Falls Auth-Seiten nicht indexiert werden sollen, `robots` bewusst in der jeweiligen Page-Metadata setzen und im Plan festhalten.
